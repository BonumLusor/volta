-- --------------------------------------------------------
--
-- Componente da OP Liberada com unidade em m²
--
-- --------------------------------------------------------
DROP TEMPORARY TABLE IF EXISTS _op;

CREATE TEMPORARY TABLE _op AS
SELECT
  -- Pedido
  op.cod_empresa,
  op.cod_pedido,
  ...
  -- OP
  op.cod_op,
  CAST( op.qtd AS UNSIGNED) AS qtd_op,
	
  -- Matéria Prima da OP
  mp.cod_produto      AS cod_produto,
  prod.desc_comercial AS nome_produto,

  -- Valida Quantidade em m² e a Unidade do Componente da OP
  mp.qtd  AS qtd_mp,
  prod.un AS un_mp,
